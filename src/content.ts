...
/* A "Shipped: 195 changes merged to production" fact lived here for one commit
   and was removed deliberately — not because it was wrong (it was counted, and
   conservatively) but because it was the wrong KIND of number for this page.
   It measured activity rather than impact, a recruiter cannot verify it, and it
   sat directly under a paragraph about knowing what a change breaks before you
   make it — a quality claim with a velocity number stapled to it. The published
   app and the timeline already carry the weight. Don't put it back. */
...
export const TIMELINE: TimelineNode[] = [
  {
    ...
    bullets: {
      en: [
        'Work the whole path a product takes: a Shopify webhook lands, queued BullMQ jobs map the catalog onto each channel’s product schema, Google Merchant Center and Meta Catalog are called, and the approval verdict comes back. One NestJS codebase deploys as a dozen specialised worker pods, with MySQL as the source of truth and MongoDB holding the per-channel read models.',
        'The area of the product I have worked on most is the bulk feed-error and disapproval workflow, across both the API and the React admin — turning raw Google and Meta rejection codes into a triaged issue list with a fix path, instead of error strings a merchant cannot act on.',
        'Made “pending approval” honest. A channel can take minutes to index a catalog it has already accepted, so scanning too early reported rejections that had never happened; I replaced that one-shot verdict with a retry chain and an ETA the merchant can actually see.',
        'Shipped the feed-rule internals merchants configure: the Shopify → Google product-category mapping, measured at 99.5% coverage against the two published taxonomies rather than estimated, plus per-market language and currency resolution for multi-market feeds.',
        'Build the React admin against an API client generated from the backend’s OpenAPI document, so a contract change is coordinated across both repos in one change instead of breaking the client silently. Embedded Shopify surfaces are built with Polaris and the Admin GraphQL API.',
      ...
      vi: [
        'Làm trọn đường đi của một sản phẩm: webhook Shopify bắn về, các job BullMQ xếp hàng ánh xạ catalog sang schema sản phẩm của từng kênh, gọi Google Merchant Center và Meta Catalog, rồi nhận phán quyết duyệt trả về. Một codebase NestJS triển khai thành hơn chục pod worker chuyên biệt, MySQL là nguồn sự thật còn MongoDB giữ read-model theo từng kênh.',
        'Mảng tôi làm nhiều nhất trong sản phẩm là luồng xử lý lỗi feed và sản phẩm bị từ chối hàng loạt, trên cả API lẫn giao diện quản trị React — biến các mã từ chối thô của Google và Meta thành danh sách lỗi đã phân loại kèm đường sửa, thay vì những chuỗi lỗi merchant không biết làm gì.',
        'Làm cho trạng thái “chờ duyệt” trung thực. Một kênh có thể mất vài phút để index đúng catalog nó vừa nhận, nên scan quá sớm sẽ báo những lỗi từ chối chưa từng xảy ra; tôi thay phán quyết một-lần-ăn-ngay đó bằng chuỗi retry kèm ETA merchant nhìn thấy được.',
        'Triển khai phần lõi của feed rule mà merchant cấu hình: bảng ánh xạ category Shopify → Google, đo được 99,5% độ phủ trên hai bộ taxonomy đã công bố chứ không phải ước lượng, cùng với việc phân giải ngôn ngữ và tiền tệ theo từng market cho feed đa thị trường.',
        'Giao diện quản trị React build trên API client sinh ra từ tài liệu OpenAPI của backend, nên mọi thay đổi hợp đồng được đồng bộ hai repo trong cùng một thay đổi thay vì làm hỏng client âm thầm. Các màn Shopify nhúng dựng bằng Polaris và Admin GraphQL API.',
    ...
    products: [
      {
        ...
        blurb: {
          en: "Creates and syncs product feeds to Google Shopping, Meta Catalog, TikTok, Bing, Pinterest and custom XML from one dashboard — prices and inventory updating in real time, feed errors surfaced early, and product disapprovals fixable in bulk. I've worked on it since my internship, across the NestJS API, its queue workers and the React admin.",
          vi: 'Tạo và đồng bộ feed sản phẩm sang Google Shopping, Meta Catalog, TikTok, Bing, Pinterest và XML tuỳ chỉnh từ một dashboard — giá và tồn kho cập nhật thời gian thực, lỗi feed được phát hiện sớm, và các sản phẩm bị từ chối xử lý được hàng loạt. Tôi làm trên sản phẩm này từ kỳ thực tập, trên cả API NestJS, các worker hàng đợi của nó lẫn giao diện quản trị React.',
        },
      },
      {
        ...
        blurb: {
          en: 'Generates a separate XML product feed per marketing channel, location or collection, each structured to the receiving catalog’s requirements. The same domain on a second commerce platform — which is why I can say the domain knowledge transfers, not just the codebase.',
          vi: 'Sinh feed sản phẩm XML riêng cho từng kênh marketing, từng địa điểm hoặc từng bộ sưu tập, mỗi feed dựng đúng yêu cầu của catalog nhận nó. Cùng lĩnh vực đó trên một nền tảng thương mại thứ hai — đó là lý do tôi có thể nói cái chuyển giao được là hiểu biết lĩnh vực, không chỉ là codebase.',
        },
      },
    ],
  },
